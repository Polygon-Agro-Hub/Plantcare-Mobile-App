...
const RequestReview: React.FC<RequestReviewProps> = ({ navigation, route }) => {
  ...
  const requestStatus = status?.toLowerCase() || request?.reqStatus?.toLowerCase() || 'pending';
    ...
    switch (requestStatus) {
      case 'approved':
        return {
          ...
          message: t("Govicapital.Your project has been successfully published on the GoViCapital platform"),
      ...
      case 'rejected':
        return {
          ...
          message: t("Govicapital.We're sorry to inform you that your project request to GoViCapital has been declined"),
      ...
      default: // pending, under_review
        return {
          ...
          message: t("Govicapital.We're taking a closer look at your request and will update you soon"),
          showSecondButton: false,
          animation: 'hourglass'
        };
    }
    ...
    <View className="absolute w-full left-0 right-0 items-center justify-center">
      ...
      >
        ...
        <View className="flex-1 items-center justify-center px-6 py-8">
          ...
          {/* {(requestStatus === 'pending' || requestStatus === 'under_review') && (
            <Text className="text-base text-[#4B6B87] text-center px-4">
              {t("Govicapital.This process might take a day or two")}
            </Text>
          )} */}

          {/* Additional Info for Rejected */}
          {/* {requestStatus === 'rejected' && (
            <Text className="text-base text-[#4B6B87] text-center px-4 mt-1">
              {t("Govicapital.Please feel free to try again in the future")}
            </Text>
          )} */}

          {/* Request Details Card (Optional) */}
          {/* {request && (
            <View className="w-full bg-gray-50 rounded-2xl p-4 mt-8">
              ...
              <View className="flex-row items-center justify-between">
                <Text className="text-sm text-gray-600">
                  {t("Govicapital.Investment")}
                </Text>
                <Text className="text-sm font-semibold text-gray-900">
                  Rs. {parseFloat(request.investment).toLocaleString('en-LK', { minimumFractionDigits: 2 })}
                </Text>
              </View>
            </View>
          )} */}
