...
import { setFarmBasicDetails, selectFarmBasicDetails, clearFarmBasicDetails, clearFarmSecondDetails } from "../../store/farmSlice";
import type { RootState , AppDispatch} from "../../services/reducxStore";
import { useTranslation } from "react-i18next";
import i18n from "@/i18n/i18n";
import { useCallback } from 'react';

// Define route params type - extends the base type
...
const AddNewFarmBasicDetails: React.FC = () => {
  ...
  // Get route parameters - cast to include fromSecondScreen
  ...
  // Initialize state - will be updated by useFocusEffect
  ...
  // Handle screen focus - reset or restore data based on navigation
 // ============================================
// 1. UPDATE AddNewFarmBasicDetails.tsx
// ============================================
...
useFocusEffect(
  useCallback(() => {
    ...
    // Check if coming from SecondDetails screen
    const isComingFromSecondScreen = 
      fromSecondScreen || 
      previousRoute?.name === 'AddNewFarmSecondDetails';
    
    if (isComingFromSecondScreen && existingFarmDetails) {
      // Restore data from Redux when coming back from second screen
      console.log('Restoring BasicDetails data from Redux...');
      ...
      // Clear form when coming from dashboard or initial navigation
      console.log('Clearing BasicDetails form - fresh start');
      setFarmName('');
      setExtentha('');
      setExtentac('');
      setExtentp('');
      ...
      // Clear both basic and second details from Redux
  ...
  // Navigate with flag indicating we're going forward
  navigation.navigate('AddNewFarmSecondDetails' as any, {
    membership: membership,
    fromFirstScreen: true  // This tells second screen it's coming from first screen
  });
};
    ...
    <KeyboardAvoidingView style={{ flex: 1 , backgroundColor: 'white' }} behavior={Platform.OS === 'ios' ? 'padding' : "padding"}>
      <View className="flex-1 bg-white">
        ...
        >
          ...
          <View className="" style={{ paddingHorizontal: wp(4), paddingVertical: hp(2) }}>
