...
const UpdateAsset: React.FC<Props> = ({ navigation, route }) => {
    ...
    useEffect(() => {
        if (tools.length > 0) {
            const initialAsset = tools[0]?.id ? updatedDetails[tools[0].id]?.asset : null;
    ...
    const ownershipCategories = [
        { key: "1", value: "Own Building (with title ownership)",translationKey: t("FixedAssets.ownBuilding") },
        { key: "2", value: "Leased Building", translationKey: t("FixedAssets.leasedBuilding") },
        { key: "3", value: "Permit Building", translationKey: t("FixedAssets.permitBuilding") },
        { key: "4", value: "Shared / No Ownership", translationKey: t("FixedAssets.sharedOwnership") },
      ];
    ...
    const landownershipCategories = [
        ...
        { key: "3", value: "Permited", translationKey: t("FixedAssets.PermittedLand") },
        { key: "4", value: "Shared", translationKey: t("FixedAssets.SharedOwnership") },
    ]
    ...
    const brands = [
      ...
      const assetTypesForBuilding = [
        ...
        { key: "3", value: "Greenhouse structure", translationKey: t("FixedAssets.greenhouseStructure") },
        { key: "4", value: "Storage facility", translationKey: t("FixedAssets.storageFacility") },
        { key: "5", value: "Storage shed", translationKey: t("FixedAssets.storageShed") },
        { key: "6", value: "Processing facility", translationKey: t("FixedAssets.processingFacility") },
        { key: "7", value: "Packing shed", translationKey: t("FixedAssets.packingShed") },
        { key: "8", value: "Dairy parlor", translationKey: t("FixedAssets.dairyParlor") },
        { key: "9", value: "Poultry house", translationKey: t("FixedAssets.poultryHouse") },
        { key: "10", value: "Livestock shelter", translationKey: t("FixedAssets.livestockShelter") },
    ];
    ...
    const handleUpdateTools = async () => {
        try {
            ...
            Alert.alert(t("FixedAssets.successTitle"), t("FixedAssets.assetsUpdatedSuccessfully"));
            navigation.goBack(); // Go back after successful update
        } catch (error) {
            console.error('Error updating assets:', error);
            Alert.alert(t("FixedAssets.errorTitle"), t("FixedAssets.failToUpdateAssets"));
        }
    };
    ...
    const handleDateChange = (toolId: string, field: string, selectedDate: Date | undefined) => {
        if (selectedDate) {
            setUpdatedDetails((prev: any) => ({
                ...prev,
                [toolId]: {
            ...
            {loading ? (
                ...
                tools.map((tool) => (
                    <View key={tool.id} className="mb-4 p-4 bg-white rounded">
                        ...
                        <Text className="font-bold text-lg text-center">{t("FixedAssets.edit")}</Text>
                        <Text className="font-bold text-lg text-center">{tool.category}</Text>

                        {tool.category === 'Land' && (
                            <> 
                            <Text className='pl-[20px] font-bold '>{t("FixedAssets.district")}</Text>
                                ...
                                <Text className='pl-[20px] font-bold '>{t("FixedAssets.ha")}</Text>
                                <TextInput
                                ...
                                />
                                <Text className='pl-[20px] font-bold'>{t("FixedAssets.ac")}</Text>
                                <TextInput
                                ...
                                />
                                <Text className='pl-[20px] font-bold'>{t("FixedAssets.p")}</Text>
                                <TextInput
...
<View className="items-center flex-row mr-3">
    <TextInput
      className="border border-gray-300 p-2 w-[70px] rounded-2xl bg-gray-100"
      value={updatedDetails[tool.id]?.extentha?.toString() || ''}
      onChangeText={(value) => handleInputChange(tool.id, 'extentha', value)}
  ...
  <View className="items-center flex-row">
    ...
    <Text className="pl-3">{t("FixedAssets.p")}</Text>
                                 ...
                                 <Text className='pl-[20px] font-bold'>{t("FixedAssets.ownership")}</Text>
                                    ...
                                    <Picker
                                        ...
                                        onValueChange={(value) => {
                                            ...
                                            if (!('oldOwnership' in updatedDetails[tool.id])) {
                                                handleInputChange(tool.id, 'oldOwnership', updatedDetails[tool.id]?.ownership || value);
                                            }
                                        }}
                                    >
                                ...
                                <Text className="pl-[20px] font-bold">{t("FixedAssets.isLandFenced")}</Text>
                                <View className="flex-row justify-around mb-5">
                                  <TouchableOpacity
                                    onPress={() => handleInputChange(tool.id, 'landFenced', 'yes')}
                                    className="flex-row items-center"
                                  >
                                    <View
                                      className={`w-5 h-5 rounded-full ${updatedDetails[tool.id]?.landFenced === "yes" ? "bg-green-500" : "bg-gray-400"}`}
                                    />
                                    <Text className="ml-2">{t("FixedAssets.yes")}</Text>
                                  </TouchableOpacity>
                                  <TouchableOpacity
                                    onPress={() => handleInputChange(tool.id, 'landFenced', 'no')}
                                    className="flex-row items-center"
                                  >
                                    <View
                                      className={`w-5 h-5 rounded-full ${updatedDetails[tool.id]?.landFenced === "no" ? "bg-green-500" : "bg-gray-400"}`}
                                    />
                                    <Text className="ml-2">{t("FixedAssets.no")}</Text>
                                  </TouchableOpacity>
                                </View>
                                
                                <Text className="pl-[20px] font-bold">{t("FixedAssets.areThereAnyPerennialCrops")}</Text>
                                <View className="flex-row justify-around mb-5">
                                  <TouchableOpacity
                                    onPress={() => handleInputChange(tool.id, 'perennialCrop', 'yes')}
                                    className="flex-row items-center"
                                  >
                                    <View
                                      className={`w-5 h-5 rounded-full ${updatedDetails[tool.id]?.perennialCrop === "yes" ? "bg-green-500" : "bg-gray-400"}`}
                                    />
                                    <Text className="ml-2">{t("FixedAssets.yes")}</Text>
                                  </TouchableOpacity>
                                  <TouchableOpacity
                                    onPress={() => handleInputChange(tool.id, 'perennialCrop', 'no')}
                                    className="flex-row items-center"
                                  >
                                    <View
                                      className={`w-5 h-5 rounded-full ${updatedDetails[tool.id]?.perennialCrop === "no" ? "bg-green-500" : "bg-gray-400"}`}
                                    />
                                    <Text className="ml-2">{t("FixedAssets.no")}</Text>
                                  </TouchableOpacity>
                                </View>
                                ...
                                <>
                                <Text className="pl-[20px] font-bold">{t("FixedAssets.estimateValue")}</Text>
                                ...
                                />
                                        <Text className="pl-[20px] font-bold">{t("FixedAssets.issuedDate")}</Text>
                                        <TouchableOpacity
                                        ...
                                        >
                                            <Text>
                                                {updatedDetails[tool.id]?.ownershipDetails?.issuedDate
                                                    ? new Date(updatedDetails[tool.id].ownershipDetails.issuedDate).toISOString().split('T')[0]
                                            ...
                                            <DateTimePicker
                                                ...
                                                onChange={(event, selectedDate) => {
                                                    ...
                                                    if (event.type === "set" && selectedDate) {
                                                        ...
                                                        handleInputChange(tool.id, 'ownershipDetails.issuedDate', formattedDate); 
                                                    }
                                                }}
                                ...
                                {updatedDetails[tool.id]?.ownership === 'Lease' && (
                                    ...
                                    <Text className="pl-[20px] font-bold">{t("FixedAssets.startDate")}</Text>
                                    <TouchableOpacity
                                    ...
                                    >
                                        <Text>
                                            {updatedDetails[tool.id]?.ownershipDetails?.startDate
                                                ? new Date(updatedDetails[tool.id].ownershipDetails.startDate).toISOString().split('T')[0]
                                        ...
                                        <DateTimePicker
                                            ...
                                            onChange={(event, selectedDate) => {
                                                ...
                                                if (event.type === "set" && selectedDate) {
                                                    const formattedDate = selectedDate.toISOString().split('T')[0];
                                                    console.log(formattedDate)
                                                    handleInputChange(tool.id, 'ownershipDetails.startDate', formattedDate); 
                                                }
                                            }}
                                            maximumDate={new Date()} 
                                        />
                                    )}
                                    <Text className="pl-[20px] font-bold">{t("FixedAssets.duration")}</Text>
                                    <View className="flex-row gap-x-4 items-center">
                                      
                                      <TextInput
                                        placeholder={t("FixedAssets.years")}
                                        keyboardType="numeric"
                                        value={updatedDetails[tool.id]?.ownershipDetails?.durationYears?.toString() || ''}
                                        onChangeText={(value) => handleInputChange(tool.id, 'ownershipDetails.durationYears', value)}
                                      ...
                                      <TextInput
                                        ...
                                        keyboardType="numeric"
                                        value={updatedDetails[tool.id]?.ownershipDetails?.durationMonths?.toString() || ''}
                                        onChangeText={(value) => handleInputChange(tool.id, 'ownershipDetails.durationMonths', value)}
                                        className="border border-gray-300 p-2 w-[110px] rounded-full bg-gray-100 pt-2 mb-2 pl-4"
                                       />
                                      <Text className="">{t("FixedAssets.months")}</Text>
                                    </View>

                                    <Text className="pl-[20px] font-bold">{t("FixedAssets.leasedAmountAnnually")}</Text>
                                    <TextInput
                                ...
                                {updatedDetails[tool.id]?.ownership === 'Permited' && (
                                    ...
                                    <Text className="pl-[20px] font-bold">{t("FixedAssets.paymentAnnually")}</Text>
                                    <TextInput
                        ...
                        {tool.category === 'Building and Infrastructures' && (
                            <>
                                <Text className='pl-[20px] font-bold pt-10'>{t("FixedAssets.type")}</Text>
                                <View className="border border-gray-300 rounded-2xl bg-white mb-2">
                                    <Picker
                                        selectedValue={updatedDetails[tool.id]?.type || ''}
                                        onValueChange={(value) => handleInputChange(tool.id, 'type', value)}
                                    >
                                ...
                                <Text className='pl-[20px] font-bold'>{t("FixedAssets.floorAreaSqrFt")}</Text>
                                <TextInput
                                ...
                                <Text className='pl-[20px] font-bold'>{t("FixedAssets.generalCondition")}</Text>
                                <View className="border border-gray-300 rounded-2xl bg-white mb-2">
                                    <Picker
                                        selectedValue={updatedDetails[tool.id]?.generalCondition || ''}
                                        onValueChange={(value) => handleInputChange(tool.id, 'generalCondition', value)}
                                    >
                                ...
                                {updatedDetails[tool.id]?.ownership === 'Own Building (with title ownership)' && (
                                ...
                                {updatedDetails[tool.id]?.ownership === 'Permit Building' && (
                                   ...
                                   {updatedDetails[tool.id]?.ownership === 'Shared / No Ownership' && (
                                    <>
                                    <Text className="pl-[20px] font-bold">{t("FixedAssets.paymentAnnually")}</Text>
                                    <TextInput
                                        placeholder={t("FixedAssets.paymentAnnually")}
                        ...
                        {tool.category === 'Machine and Vehicles' ? (
                            <> 
                                <Text className='pl-[20px] font-bold pt-10'>{t("FixedAssets.asset")}</Text>
                                <View className="border border-gray-400 rounded-2xl bg-white mb-2">
                                    <Picker
                                ...
                                {selectedAsset && assetTypesForAssets[selectedAsset] && (
                                    <>
                                        <Text className='pl-[20px] font-bold'>{t("FixedAssets.assetType")}</Text>
                                        <View className="border border-gray-400 rounded-2xl bg-white mb-2">
                                            <Picker
                                                selectedValue={updatedDetails[tool.id]?.assetType || ''}
                                                onValueChange={(value) => handleInputChange(tool.id, 'assetType', value)}
                                            >
                                ...
                                {updatedDetails[tool.id]?.assetType === 'Other' && (
                                    <>
                                    <Text className='pl-[20px] font-bold'>{t("FixedAssets.mentionOther")}</Text>
                                      <TextInput
                                ...
                                {selectedAsset && brandTypesForAssets[selectedAsset] && (
                                    <>
                                        <Text className='pl-[20px] font-bold'>{t("FixedAssets.brand")}</Text>
                                        <View className="border border-gray-400 rounded-2xl bg-white mb-2">
                                            <Picker
                                            ...
                                            >
                                                {brandTypesForAssets[selectedAsset].map((brand: any) => (
                                                    <Picker.Item label={brand.value} value={brand.value} key={brand.key} />
                                                ))}
                                            </Picker>
                                        </View>
                                    </>
                                )}
                                <Text className='pl-[20px] font-bold'>{t("FixedAssets.numberofUnits")}</Text>
                                <TextInput
                                ...
                                <Text className='pl-[20px] font-bold'>{t("FixedAssets.unitPrice")}</Text>
                                <TextInput
                                    ...
                                    className="border border-gray-400 rounded-2xl p-2 mb-2 pl-4"
                                />
                                <Text className='pl-[20px] font-bold'>{t("FixedAssets.totalPrice")}</Text>
                                <TextInput
                                    ...
                                    className="border border-gray-400 rounded-2xl p-2 mb-2 pl-4"
                                />
                                <Text className='pl-[20px] font-bold'>{t("FixedAssets.warranty")}</Text>
                                <View className="border border-gray-400 rounded-2xl bg-white mb-2">
                                    <Picker
                                ...
                                {updatedDetails[tool.id]?.warranty === 'yes' && (
                                    <>
                                    <Text className='pl-[20px] font-bold'>{t("FixedAssets.purchasedDate")}</Text>
                                    <TextInput
                                    ...
                                    className="border border-gray-400 rounded-2xl p-2 mb-2 pl-4"
                                />
                                <Text className='pl-[20px] font-bold'>{t("FixedAssets.warrantyExpireDate")}</Text>
                                <TextInput
                                    ...
                                    className="border border-gray-400 rounded-2xl p-2 mb-2 pl-4"
                                />
                                <Text className='pl-[20px] font-bold'>{t("FixedAssets.warrantyStatus")}</Text>
                                <TextInput
                        ...
                        <TouchableOpacity
                            ...
                            <Text className="text-white">{t("FixedAssets.updateAsset")}</Text>
                        </TouchableOpacity>
                    </View>
                ))
            )}
