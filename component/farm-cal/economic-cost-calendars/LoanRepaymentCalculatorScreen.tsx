  ...
  const handleCalculate = () => {
    ...
    if (!loanAmount || !interestRate || !loanYears) {
      Alert.alert(
        ...
        t("EconomicCostCalendars.FillAllFields") || "Please fill all required fields.",
      );
      return;
    }
    ...
    if (isNaN(loanNum) || loanNum <= 0) {
      Alert.alert(
        t("EconomicCostCalendars.InvalidInput") || "Invalid Input",
        t("EconomicCostCalendars.LoanAmountError") || "Loan amount must be greater than 0.",
      );
      return;
    }
    if (isNaN(rateNum) || rateNum <= 0) {
      Alert.alert(
        t("EconomicCostCalendars.InvalidInput") || "Invalid Input",
        t("EconomicCostCalendars.InterestRateError") || "Interest rate must be greater than 0.",
      );
      return;
    }
    if (isNaN(yearsNum) || yearsNum <= 0) {
      Alert.alert(
        t("EconomicCostCalendars.InvalidInput") || "Invalid Input",
        t("EconomicCostCalendars.YearsError") || "Number of years must be greater than 0.",
      );
      return;
    }

    // Calculate Monthly Payment = L × (r/12) × (1 + r/12)^(n×12) / ((1 + r/12)^(n×12) - 1)
    // Convert annual interest rate to monthly (divide by 100 to get decimal, then by 12)
    ...
    if (compoundFactor !== 1) {
      monthlyPayment = loanNum * monthlyRate * compoundFactor / (compoundFactor - 1);
    } else {
  ...
  return (
    <View className="flex-1 bg-white">
      ...
      >
        {isFormInvalid && (
          <Text className="text-[#287097] text-sm font-medium mb-5">
            {t("EconomicCostCalendars.FillRequiredFields") || "Please fill all required fields!"}
          </Text>
        )}

        {/* Loan Amount Input - 2 decimal points */}
        <Text className="text-sm font-semibold text-gray-900 mb-2">
          {t("EconomicCostCalendars.LoanAmount") || "Loan Amount (Rs.)"} *
        </Text>
        <TextInput
          value={loanAmount}
          onChangeText={(text) => handleNumberInput(text, setLoanAmount, 2)}
        ...
        {/* Interest Rate Input - 2 decimal points */}
        <Text className="text-sm font-semibold text-gray-900 mb-2">
        ...
        {/* Number of Years Input - 1 decimal point */}
        <Text className="text-sm font-semibold text-gray-900 mb-2">
      ...
      <ResultModal
        ...
        cropName={t("EconomicCostCalendars.MonthlyPayment") || "Monthly Payment"}
