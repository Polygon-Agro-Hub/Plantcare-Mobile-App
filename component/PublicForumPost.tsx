...
type PublicForumPostNavigationProp = StackNavigationProp<RootStackParamList, 'PublicForumPost'>;
...
//   const navigation = useNavigation<PublicForumPostNavigationProp>();
  ...
  const handleSubmit = async () => {
    ...
    if (postImageUri) {
        ...
        const fileType = fileName?.split('.').pop() ? `image/${fileName.split('.').pop()}` : 'image/jpeg'; // Default to jpeg
    ...
    try {
        const response = await axios.post('http://10.0.2.2:3000/api/auth/add/post', formData, {
            headers: {
    ...
    <View className="flex-1 bg-white">
      ...
      <View className="px-4 py-6 p-7">
        ...
        <View className="mb-4 items-center mt-[5%]">
          ...
          {postImageUri && <Image source={{ uri: postImageUri }} className="w-32 h-32 mt-4" />}
        </View>
