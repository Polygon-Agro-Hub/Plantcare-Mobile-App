...
const Signin: React.FC<SigninProps> = ({ navigation }) => {
    ...
    const handleLogin = async () => {
        if (!phonenumber) {
            Alert.alert(t('signinForm.validationError'), t('signinForm.phoneNumberRequired'));
            return;
        }
        ...
        try {
            ...
            if (contentType && contentType.includes('application/json')) {
                ...
                if (data.token) {
                    ...
                    Alert.alert(t('signinForm.loginFailed'), t('signinForm.noTokenReceived'));
                }
            } else {
                Alert.alert(t('signinForm.error'), t('signinForm.expectedJson'));
            }
        } catch (error) {
            Alert.alert(t('signinForm.loginFailed'), t('signinForm.somethingWentWrong'));
        ...
        <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
            <View className='flex-1 bg-white'>
                ...
                <View className='items-center'>
                    ...
                    <Text className='pt-[35px] text-base'>{t('signinForm.enteryourphno')}</Text>
